...
export function extractGroupKey(
  ...
  switch (rule.type) {
    case "folder": {
      ...
      const folderParts = depth > 1 ? parts.slice(0, Math.min(depth, parts.length - 1)) : [parts[0]]
 ...
 */
export function isRuleEffective<T extends { slug?: string; frontmatter?: Record<string, unknown> }>(
  items: T[],
  rule: AggregationRule,
): boolean {
  let hasValid = false
 ...
 * 执行单条聚合规则，返回分组 Map
 */
export function applyAggregationRule<T extends { slug?: string; frontmatter?: Record<string, unknown> }>(
  items: T[],
  rule: AggregationRule,
): Map<string, T[]> {

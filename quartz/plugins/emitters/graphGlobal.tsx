// graphGlobal.tsx - 全局图谱构建时预计算生成器
//
// 功能：在构建时执行全局图谱的完整计算流程，输出 graphGlobal.json，
//       运行时直接加载预计算结果，跳过所有耗时计算。
//
// 预计算流程与 graph3.inline.ts 中的运行时逻辑完全一致：
// 1. 虚拟节点计算
// 2. 全局链接图构建
// 3. 节点/链接去重 + 孤儿过滤
...
//
// 注意：此 emitter 依赖 ContentIndex 已先生成 contentIndex.json，
// 因此在 quartz.config.ts 的 plugins.emitters 中必须排在 ContentIndex 之后。
// 同时依赖 quartz.layout.json 中 graph 相关配置已合并到 ctx.cfg.configuration.graph 中。
...
import { CoreNodeFilterConfig } from "../../util/aggregation"
...
interface Options {
  enabled?: boolean
  /** 边缘节点聚合规则（与 quartz.layout.json 中 graph.aggregation 一致） */
  aggregation?: AggregationRule[]
  /** 大区聚合规则（与 quartz.layout.json 中 graph.regionRules 一致） */
  regionRules?: AggregationRule[]
  /** 核心节点过滤规则（与 quartz.layout.json 中 graph.coreNodeFilter 一致） */
  coreNodeFilter?: CoreNodeFilterConfig
  /** 核心节点数量硬上限 */
  coreNodeLimit?: number
  /** 全局图谱是否默认收起 */
...
export const GraphGlobal: QuartzEmitterPlugin<Partial<Options>> = (opts) => {
  ...
  async function* computeAndEmit(
    ctx: BuildCtx,
    contentData: Map<SimpleSlug, ContentDetails>,
  ) {
    ...
    const totalStart = performance.now()
      ...
      const allLinks = rawLinks.filter((l) => neighbourhood.has(l.source) && neighbourhood.has(l.target))
        .filter((l) => {
      ...
      // 过滤孤儿节点
      const nonOrphanNodeIds = new Set([...neighbourhood].filter((id) => (nodeLinkCount.get(id) ?? 0) > 0))
      ...
      if (coreNodeLimit && coreNodeLimit > 0 && !(regionRules && regionRules.length > 0)) {
        if (coreNodeIdSet.size > coreNodeLimit) {
          const sorted = [...coreNodeIdSet].sort((a, b) => (nodeLinkCount.get(b) ?? 0) - (nodeLinkCount.get(a) ?? 0))
      ...
      for (let li = 0; li < effectiveLinks.length; li++) {
        ...
        if (srcIsEdge && !tgtIsEdge) {
          ...
          if (!nodeToEdgeNodeIds[l.target].includes(l.source)) nodeToEdgeNodeIds[l.target].push(l.source)
          ...
          if (!nodeToEdgeNodeIds[l.source].includes(l.target)) nodeToEdgeNodeIds[l.source].push(l.target)
      ...
      // childToAgg 使用复合键 `${coreId}\t${edgeId}` → aggId，
      // 支持 hasRegionRules=true 时多归属节点被多个核心独立聚合
      ...
      if (rules.length > 0) {
        for (const [coreId, edgeNodeIds] of Object.entries(nodeToEdgeNodeIds)) {
          ...
          for (let ruleIdx = 0; ruleIdx < rules.length; ruleIdx++) {
            ...
            if (rule.type !== "folder") {
              if (!isRuleEffective(
                leavesForNextRule.map((id) => ({ slug: id, frontmatter: contentData.get(id as SimpleSlug)?.frontmatter })),
                rule,
              )) continue
            }
      ...
      if (regionRules && regionRules.length > 0) {
        ...
        for (const [aggId, info] of Object.entries(aggNodes)) {
          ...
          firstScreenLinks.push({ source: aggId, target: info.coreId, sourceField: info.currentField })
        }
      }
      console.log(`  [Step 10] First screen: ${firstScreenNodeIds.length} nodes, ${firstScreenLinks.length} links`)
      ...
      const fp = joinSegments("graph", "global", "graphGlobal") as any
